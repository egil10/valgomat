...
export function FeaturedStatement() {
  ...
  return (
    <div className="glass-strong rounded-3xl p-5 sm:p-6" aria-live="polite">
      <p className="text-[11px] uppercase tracking-[0.18em] text-ink/55">
        Smakebit · {q.topic}
      </p>
      <p className="mt-2 font-display text-xl font-medium leading-snug text-balance text-ink sm:text-2xl">
        «{q.statement}»
      </p>
      <div className="mt-4 grid grid-cols-7 gap-1.5" aria-hidden>
        {Array.from({ length: 7 }, (_, k) => k + 1).map((score) => {
          ...
          return (
            <div key={score} className="flex min-h-[80px] flex-col items-center gap-1 rounded-xl border border-black/[0.05] bg-white/40 px-1 pb-2 pt-1.5">
